...
// Comprehensive validation schema for all modification types
const validationSchema = yup.object({
  ...
  // Optional fields that will be validated based on modification type
  newRate: yup.number().when('modificationType', {
    is: 'RATE_CHANGE',
    then: (schema) => schema.required('New rate is required').min(0.01, 'Rate must be greater than 0').max(50, 'Rate cannot exceed 50%'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  additionalMonths: yup.number().when('modificationType', {
    is: 'TERM_EXTENSION',
    then: (schema) => schema.required('Additional months is required').min(1, 'Must extend by at least 1 month').max(360, 'Cannot extend by more than 360 months'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  newPaymentAmount: yup.number().when('modificationType', {
    is: (val: string) => ['PAYMENT_REDUCTION_TEMPORARY', 'PAYMENT_REDUCTION_PERMANENT'].includes(val),
    then: (schema) => schema.required('New payment amount is required').min(1, 'Payment must be greater than 0'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  numberOfTerms: yup.number().when('modificationType', {
    is: 'PAYMENT_REDUCTION_TEMPORARY',
    then: (schema) => schema.required('Number of terms is required').min(1, 'Must be at least 1 term').max(60, 'Cannot exceed 60 terms'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  reductionAmount: yup.number().when('modificationType', {
    is: 'PRINCIPAL_REDUCTION',
    then: (schema) => schema.required('Reduction amount is required').min(1, 'Reduction must be greater than 0'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  balloonAmount: yup.number().when('modificationType', {
    is: 'BALLOON_PAYMENT_ASSIGNMENT',
    then: (schema) => schema.required('Balloon amount is required').min(1, 'Balloon amount must be greater than 0'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  balloonDueDate: yup.date().when('modificationType', {
    is: 'BALLOON_PAYMENT_ASSIGNMENT',
    then: (schema) => schema.required('Balloon due date is required'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  customStartTerm: yup.number().when('reamortizationStartType', {
    is: 'CUSTOM',
    then: (schema) => schema.required('Custom start term is required').min(1),
    otherwise: (schema) => schema.notRequired()
  }),
  
  durationMonths: yup.number().when('modificationType', {
    is: (val: string) => ['FORBEARANCE', 'DEFERMENT'].includes(val),
    then: (schema) => schema.required('Duration is required').min(1, 'Duration must be at least 1 month'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  eligibilityReason: yup.string().when('modificationType', {
    is: 'DEFERMENT',
    then: (schema) => schema.required('Eligibility reason is required'),
    otherwise: (schema) => schema.notRequired()
  }),
  
  // Optional fields
  keepSamePayment: yup.boolean().notRequired(),
  interestHandling: yup.string().notRequired(),
  termAdjustment: yup.string().notRequired(),
  newTermMonths: yup.number().notRequired(),
  principalReduction: yup.number().notRequired(),
  paymentRecalculation: yup.string().notRequired(),
  reamortizationStartType: yup.string().notRequired(),
  reamortizationType: yup.string().notRequired(),
  forbearanceType: yup.string().notRequired(),
  reducedPaymentAmount: yup.number().notRequired(),
  interestSubsidy: yup.boolean().notRequired(),
  newInterestRate: yup.number().notRequired(),
});
...
export const LoanModificationModal = ({ 
  loan, 
  isOpen, 
  onClose, 
  onSuccess 
